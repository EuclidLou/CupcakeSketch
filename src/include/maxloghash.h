...
public:
    ...
    void insert(data_t item)
    {
        ...
        for (int i = 0; i < HASH_CNT; i++)
        {
            ...
            LOG_DEBUG("hash_value: %lu", hash_value);
...
double similarity_maxloghash_cm(MAXLOGHASH_CM *maxloghash1, MAXLOGHASH_CM *maxloghash2)
{
    ...
    {
        LOG_DEBUG("maxloghash1->max[%d]: %lu, maxloghash1->is_only[%d]: %d, maxloghash2->max[%d]: %lu, maxloghash2->is_only[%d]: %d", i, maxloghash1->max_hash_value[i], i, maxloghash1->is_the_only_max[i], i, maxloghash2->max_hash_value[i], i, maxloghash2->is_the_only_max[i]);
